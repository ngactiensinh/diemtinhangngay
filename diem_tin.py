"""
HỆ THỐNG ĐIỂM TIN & LẮNG NGHE DƯ LUẬN - PHIÊN BẢN V5.6 (LOCAL FOCUSED)
Đã vá: Nhúng bộ từ khóa 124 xã, phường của Tỉnh vào lõi lọc tin tức Dư luận
"""
...
RSS_FEEDS = {
    ...
    # Hút tổng thể MXH về Tuyên Quang, code Python sẽ lo việc rây lọc lại đúng 124 xã
    "🗣️ Dư luận MXH": {"url": "https://news.google.com/rss/search?q=(%22facebook%22+OR+%22fanpage%22+OR+%22m%E1%BA%A1ng+x%C3%A3+h%E1%BB%99i%22)+%22Tuy%C3%AAn+Quang%22+when:30d&hl=vi&gl=VN&ceid=VN:vi", "tag": "MẠNG XÃ HỘI"},
    "📍 Địa phương": {"url": "https://news.google.com/rss/search?q=%22Tuy%C3%AAn+Quang%22+when:3d&hl=vi&gl=VN&ceid=VN:vi", "tag": "ĐỊA PHƯƠNG"},
...
# ==========================================
# BỘ TỪ KHÓA 124 XÃ/PHƯỜNG VÀ CÁC HUYỆN
# ==========================================
LOCAL_KEYWORDS = [
...
def score_entry(entry):
    ...
    # Chấm điểm ưu tiên dựa trên bộ từ khóa của tỉnh
...
def fetch_rss(url, require_local=False):
    try:
        ...
        for e in feed.entries:
            if not is_safe(e): continue
            
            # LƯỚI LỌC 124 XÃ/PHƯỜNG: 
            # Nếu là tab Dư luận/Địa phương, chỉ giữ lại tin nhắc đến Tuyên Quang hoặc các xã/huyện
            if require_local:
...
for i, (tab_name, cfg) in enumerate(RSS_FEEDS.items()):
    with tabs[i]:
        with st.spinner(f"Đang xử lý dữ liệu {tab_name}..."):
            # Bật màng lọc 124 đơn vị cho các tab nhạy cảm
            req_loc = True if tab_name in ["🗣️ Dư luận MXH", "🤝 Dân vận khéo", "📍 Địa phương"] else False
            entries = fetch_rss(cfg["url"], require_local=req_loc)

        if not entries:
            st.info("📭 Chưa có bài viết/bình luận nào trên không gian mạng liên quan đến Tuyên Quang trong 30 ngày qua.")
            continue
